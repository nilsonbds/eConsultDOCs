---
sidebar_position: 0
---

# Começando no eConsult: Terapia Individual

🚀 Bem-vindo ao eConsult.

Este guia foi criado para ajudar você a começar a utilizar o sistema de forma prática, leve e organizada no seu dia a dia clínico.

Você não precisa configurar tudo agora.

O mais importante é começar a utilizar o sistema na prática.

⏱️ Em menos de 15 minutos você já consegue:

- cadastrar pacientes
- organizar sua agenda
- registrar atendimentos
- acompanhar evoluções clínicas

## 🔐 Como acessar o eConsult

Para abrir o eConsult, basta acessar o site oficial da plataforma:

👉 [eConsult - econsult.app.br](https://econsult.app.br/home)

Depois:

1. Clique no botão **Entrar** no topo da tela. 
3. Informe seu email e senha
4. Acesse sua conta

## 📲 Primeira coisa: instale o app do eConsult no seu dispositivo

Você pode instalar o eConsult no seu celular, tablet ou computador para acessar o sistema com mais rapidez e praticidade no dia a dia clínico.

Para instalar:

1. Acesse o painel principal do eConsult
2. No topo da tela, clique em **Instalar APP**
3. Siga as instruções exibidas no seu dispositivo
   
---

## 🚀 Sua jornada inicial no eConsult

### 1. Complete o cadastro da sua conta

Antes de começar, recomendamos complementar as informações da sua conta no eConsult.

Isso ajuda a personalizar melhor sua experiência dentro da plataforma, além de preparar funcionalidades importantes como agenda, documentos, teleatendimento, financeiro e relatórios.

Você poderá configurar informações como:

- dados profissionais e pessoais (importante)
- endereços
- foto de perfil
- logotipo do seu consultório

##### 🎞️ Vídeo rápido

- acessando a conta  
- abrindo “Dados Gerais”  
- salvando as informações  
- cadastrando um endereço 

<video controls preload="metadata" poster="/img/thumbs/video-tutorial.jpg" style={{  borderRadius: '15px', margin: '1.5rem 0', maxHeight: '670px', maxWidth: '100%', border: '1px solid whitesmoke', boxShadow: '0 10px 30px rgba(15, 23, 42, 0.10)', overflow: 'hidden', background: '#000', cursor: 'pointer' }}>
  <source src="/video/conta.mp4" type="video/mp4" />
  Seu navegador não suporta vídeo.
</video> 

👉 [se precisar veja mais detalhes sobre configurações de conta](./01_conta.md)

---

### 2. Cadastre seus pacientes

Comece cadastrando seus pacientes ativos no eConsult.

Se você também realiza atendimentos com casais, famílias ou grupos terapêuticos, consulte os guias específicos:

- 👉 [Primeiros passos: Terapia de Casais](#)
- 👉 [Primeiros passos: Terapia Familiar](#)
- 👉 [Primeiros passos: Grupos Terapêuticos](#)


##### 🎞️ Vídeo rápido

- cadastrando um paciente  

<video controls preload="metadata" poster="/img/thumbs/video-tutorial.jpg" style={{  borderRadius: '15px', margin: '1.5rem 0', maxHeight: '670px', maxWidth: '100%', border: '1px solid whitesmoke', boxShadow: '0 10px 30px rgba(15, 23, 42, 0.10)', overflow: 'hidden', background: '#000', cursor: 'pointer' }}>
  <source src="/video/individual/pacientes.mp4" type="video/mp4" />
  Seu navegador não suporta vídeo.
</video>

:::note
Repare que basta informar o nome e o tipo de atendimento para cadastrar um paciente.

As demais informações podem ser preenchidas mais tarde, conforme a necessidade. Isso torna o cadastro inicial muito mais rápido e prático.
:::

👉 [veja mais detalhes sobre cadastro de pacientes](./02_pacientes.md)

---

### 3. Organize sua agenda do mês

Utilize o "**agendamentos múltiplos**" para montar sua agenda de forma rápida.

Você pode:
- Criar atendimentos recorrentes
- Organizar sua rotina mensal
- Visualizar sua disponibilidade

##### 🎞️ Vídeo rápido

- fazendo agendamentos múltiplos do paciente

<video controls preload="metadata" poster="/img/thumbs/video-tutorial.jpg" style={{  borderRadius: '15px', margin: '1.5rem 0', maxHeight: '670px', maxWidth: '100%', border: '1px solid whitesmoke', boxShadow: '0 10px 30px rgba(15, 23, 42, 0.10)', overflow: 'hidden', background: '#000', cursor: 'pointer' }}>
  <source src="/video/individual/agendamento-de-multiplos-atendimentos.mp4" type="video/mp4" />
  Seu navegador não suporta vídeo.
</video>

👉 [veja mais detalhes sobre agenda](./03_agendamento-de-multiplos-atendimentos.md)

---

### 4. Verifique próximas ações no painel Situações de Atendimento

✨ Esse painel funciona como um centro de acompanhamento da sua rotina clínica.

Uma vez que você fez agendamentos de atendimentos, já é possível verificar ações que podem ser feitas através do painel **Situações de Atendimento**

Esse painel reúne, em uma única tela, tudo o que precisa da sua atenção no dia a dia:

- Atendimentos que precisam de confirmação  
- Registros de realização pendentes  
- Pagamentos em aberto  
- Pacientes sem novo agendamento
- etc.  

👉 Cada item funciona como um **indicador de ação**, ajudando você a identificar rapidamente o que precisa ser resolvido.

Você pode clicar em cada situação para ver os detalhes e executar ações como:

- confirmar atendimentos  
- registrar sessões realizadas  
- acompanhar pagamentos  
- entrar em contato com pacientes  

##### 🎞️ Vídeo rápido

- explorando o painel situações de atendimento

<video controls preload="metadata" poster="/img/thumbs/video-tutorial.jpg" style={{  borderRadius: '15px', margin: '1.5rem 0', maxHeight: '670px', maxWidth: '100%', border: '1px solid whitesmoke', boxShadow: '0 10px 30px rgba(15, 23, 42, 0.10)', overflow: 'hidden', background: '#000', cursor: 'pointer' }}>
  <source src="/video/individual/situacoes-de-atendimento-clinico.mp4" type="video/mp4" />
  Seu navegador não suporta vídeo.
</video>

👉 [mais detalhes sobre situações de atendimento](./04_situacoes-de-atendimento-clinico.md).

---

### 5. Explore o painel de resultados

No painel "**Resultados**", você acompanha:

- Movimentações financeiras
- Indicadores administrativos
- Visão geral da sua operação

##### 🎞️ Vídeo rápido

- explorando o painel resultados

<video controls preload="metadata" poster="/img/thumbs/video-tutorial.jpg" style={{  borderRadius: '15px', margin: '1.5rem 0', maxHeight: '670px', maxWidth: '100%', border: '1px solid whitesmoke', boxShadow: '0 10px 30px rgba(15, 23, 42, 0.10)', overflow: 'hidden', background: '#000', cursor: 'pointer' }}>
  <source src="/video/resultados.mp4" type="video/mp4" />
  Seu navegador não suporta vídeo.
</video>

👉 [mais detalhes sobre o painel de resultados](./05_resultados.md).

---

### 6. Registre a evolução clínica com marcadores clínicos

✨ Um dos principais diferenciais do eConsult é justamente ajudar o profissional a acompanhar a evolução clínica do paciente ao longo do tempo.

Após, ou durante a realização de cada atendimento, registre a evolução do atendimento.

No eConsult, isso pode ser feito com apoio dos **marcadores clínicos**, que ajudam a estruturar:

- Engajamento
- Risco
- Evolução
- Prioridades clínicas

##### 🎞️ Vídeo rápido

- selecionando marcadores clínicos para um atendimento

<video controls preload="metadata" poster="/img/thumbs/video-tutorial.jpg" style={{  borderRadius: '15px', margin: '1.5rem 0', maxHeight: '670px', maxWidth: '100%', border: '1px solid whitesmoke', boxShadow: '0 10px 30px rgba(15, 23, 42, 0.10)', overflow: 'hidden', background: '#000', cursor: 'pointer' }}>
  <source src="/video/individual/anotacoes-clinicas-com-marcadores-clinicos.mp4" type="video/mp4" />
  Seu navegador não suporta vídeo.
</video>

Se você selecionar apenas um marcador e adicionar uma breve descrição com suas observações clínicas, o sistema já será capaz de gerar uma anotação estruturada. Naturalmente, quanto maior o número de marcadores e observações inseridos, mais rica e detalhada será a análise produzida pelo eConsult.

👉 [aprofundar sobre marcadores clínicos](./06_anotacoes-clinicas-com-marcadores-clinicos.md).

---

### 7. Acompanhe a evolução no painel longitudinal

Utilize o **Acompanhamento Longitudinal do Paciente** para visualizar:

- Evolução ao longo do tempo
- Mudanças de padrão
- Tendências clínicas

##### 🎞️ Vídeo rápido

- verificando evolução
- mudanças de padrão
- tendências clínicas do paciente

<video controls preload="metadata" poster="/img/thumbs/video-tutorial.jpg" style={{  borderRadius: '15px', margin: '1.5rem 0', maxHeight: '670px', maxWidth: '100%', border: '1px solid whitesmoke', boxShadow: '0 10px 30px rgba(15, 23, 42, 0.10)', overflow: 'hidden', background: '#000', cursor: 'pointer' }}>
  <source src="/video/individual/acompanhamento-clinico-longitudinal.mp4" type="video/mp4" />
  Seu navegador não suporta vídeo.
</video>

👉 [mais sobre evolução e visão longitudinal](./07_acompanhamento-clinico-longitudinal.md)

---

### 8. Inicie o prontuário

Para consolidar o aprendizado, escolha um paciente e:

- Registre pelo menos 4 atendimentos
- Utilize marcadores clínicos para preencher as evoluções de cada atendimento
- Estruture as anotações em SOAP de cada atendimento

Após finalizar, inicie o prontuário do paciente.

##### 🎞️ Vídeo rápido

- registrando a anotação SOAP do atendimento
- explorando prontuário

<video controls preload="metadata" poster="/img/thumbs/video-tutorial.jpg" style={{  borderRadius: '15px', margin: '1.5rem 0', maxHeight: '670px', maxWidth: '100%', border: '1px solid whitesmoke', boxShadow: '0 10px 30px rgba(15, 23, 42, 0.10)', overflow: 'hidden', background: '#000', cursor: 'pointer' }}>
  <source src="/video/individual/prontuario-eletronico.mp4" type="video/mp4" />
  Seu navegador não suporta vídeo.
</video>

✨ No eConsult, o prontuário não precisa ser escrito do zero a cada momento — ele é construído com apoio do sistema ao longo do acompanhamento clínico.

Depois, a cada atendimento realizado, as evoluções, marcadores clínicos e anotações são incorporados automaticamente ao prontuário e ao histórico clínico do paciente.

Assim, o prontuário vai sendo estruturado progressivamente durante o processo terapêutico.

Ao final — ou em momentos importantes do acompanhamento — o profissional pode complementar o registro com hipóteses diagnósticas, prognósticas, plano terapêutico e evolução clínica do tratamento, além de publicar um recorte temporal do prontuário.

👉 [mais detalhes sobre prontuário](./08_prontuario-eletronico.md)

---

### 9. Finalize e publique o prontuário

Depois de estruturar o prontuário ao longo dos atendimentos, chegará o momento de consolidar e publicar um recorte clínico do acompanhamento.

Ao finalizar:

- o prontuário é publicado no histórico clínico
- o conteúdo daquele período fica consolidado
- você preserva uma versão organizada da evolução do paciente
- o registro pode ser mantido privado ou compartilhado com o paciente

👉 É nesse momento que o prontuário deixa de ser apenas um rascunho e passa a compor oficialmente o histórico clínico do acompanhamento.

##### 🎞️ Vídeo

- gerando hipóteses diagnóticas, prognósticas, planos e evolução terapêutica com IA
- finalizando o prontuário
- publicando como público
- mostrando prontuário na Área do Paciente

<video controls preload="metadata" poster="/img/thumbs/video-tutorial.jpg" style={{  borderRadius: '15px', margin: '1.5rem 0', maxHeight: '670px', maxWidth: '100%', border: '1px solid whitesmoke', boxShadow: '0 10px 30px rgba(15, 23, 42, 0.10)', overflow: 'hidden', background: '#000', cursor: 'pointer' }}>
  <source src="/video/individual/prontuario-finalizado.mp4" type="video/mp4" />
  Seu navegador não suporta vídeo.
</video>

⚠️ Ao publicar, você poderá definir se o prontuário será:

- 🔒 Privado → apenas você visualiza
- 🌐 Público → disponível ao paciente na Área do Paciente, se autorizado

✨ No eConsult, o prontuário é construído continuamente ao longo do acompanhamento — e a finalização representa a consolidação de um momento clínico importante do processo terapêutico.

👉 [mais detalhes sobre finalização do prontuário](./09_prontuario-finalizado.md)

---

:::tip Dica importante

Você não precisa configurar tudo no primeiro dia.

O mais importante é:

✔ Começar a usar  
✔ Registrar seus atendimentos  
✔ Explorar os painéis aos poucos  
:::

## 👉 Próximas trilhas recomendadas

### 🧩 Guias práticos por contexto clínico

#### 🧑‍🤝‍🧑 Primeiros passos: Terapia de Casal
- Cadastro do casal
- Organização dos atendimentos
- Evolução clínica compartilhada
- Prontuário longitudinal do casal

👉 [Primeiros passos: Terapia de Casais](../04-comecando-casal/00_comecando-econsult-primeiros-passos.md)

#### 👨‍👩‍👧 Primeiros passos: Terapia de Família
- Estrutura familiar
- Membros vinculados
- Sessões familiares
- Histórico e dinâmica longitudinal

👉 [Primeiros passos: Terapia de Família](../05-comecando-familia/00_comecando-econsult-primeiros-passos.md)

#### 👥 Primeiros passos: Terapia em Grupo
- Cadastro de grupos terapêuticos
- Participantes e presença
- Evolução grupal
- Organização longitudinal dos encontros

👉 [Primeiros passos: Terapia em Grupo](../06-comecando-grupo/00_comecando-econsult-primeiros-passos.md)

---

Se precisar de ajuda, você pode acessar a documentação completa ou entrar em contato com o suporte (atendimento@econsult.app.br).